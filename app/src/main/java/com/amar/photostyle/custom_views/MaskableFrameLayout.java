package com.amar.photostyle.custom_views;
...
public class MaskableFrameLayout extends FrameLayout {
    ...
    private void construct(Context context, AttributeSet attrs) {
        ...
        if (theme != null) {
            TypedArray a = theme.obtainStyledAttributes(
                    ...
                    0, 0);
            try {
                //Load the masks if specified in xml
                initMask(loadMask(a));
            ...
            log("Couldn't load theme, masks in xml won't be loaded.");
        }
        registerMeasure();
    }
    ...
    private void initMask(@Nullable Drawable input) {
        if (input != null) {
            ...
            log("Are you sure you don't want to provide a masks ?");
        }
    }
        ...
        try {
            if (drawable != null) {
                if (getMeasuredWidth() > 0 && getMeasuredHeight() > 0) {
                    ...
                    log("Can't create a masks with height 0 or width 0. Or the layout has no children and is wrap content");
                    return null;
                }
            } else {
                log("No bitmap masks loaded, view will NOT be masked !");
            }
        } catch (OutOfMemoryError | IllegalArgumentException outOfMemoryError) {
            Log.e(TAG, "makeBitmapMask: exception out of memory" );
        } catch (Exception e){
            Log.e(TAG, "makeBitmapMask: "+e );
        }
        return null;
    }
    ...
    public void setMask(int drawableRes) {
        ...
        if (res != null) {
            ...
            log("Unable to load resources, masks will not be loaded as drawable");
        }
    }
    ...
    //Once the size has changed we need to remake the masks.
    @Override
    ...
    //Once inflated we have no height or width for the masks. Wait for the layout.
